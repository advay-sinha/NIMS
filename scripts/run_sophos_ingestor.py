"""Entry point: run Sophos ingestion (firewall syslog + Central API).

Offline by default. Live mode is gated (disabled unless the source config has
enabled=true, mode=live, read_only=true). Never contacts Sophos in offline/mock.

Usage
-----
    python -m scripts.run_sophos_ingestor --offline --run-once
    python -m scripts.run_sophos_ingestor --source firewall_syslog --live --run-once
"""
...
SOPHOS_SOURCES = ["sophos_firewall_syslog", "sophos_central"]


def build_parser() -> argparse.ArgumentParser:
    """Construct the argument parser for this entry point."""
    parser = argparse.ArgumentParser(description="Run Sophos ingestion.")
    ...
    parser.add_argument("--source", default=None,
                        help="firewall_syslog | central_api (default: both).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sources = [args.source] if args.source else SOPHOS_SOURCES
    ...
    logger.info("Sophos ingestion complete: %d events, output=%s", status.total_events, output_dir)
    return 0
