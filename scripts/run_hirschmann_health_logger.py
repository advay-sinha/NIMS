"""Entry point: run Hirschmann health ingestion (SNMP metrics + traps).

Offline by default. Live mode is gated (disabled unless the source config has
enabled=true, mode=live, read_only=true). SNMP is read-only (GET only, never SET).

Usage
-----
    python -m scripts.run_hirschmann_health_logger --offline --run-once
    python -m scripts.run_hirschmann_health_logger --source snmp --live --run-once
"""
...
HEALTH_SOURCES = ["hirschmann_snmp", "hirschmann_traps"]


def build_parser() -> argparse.ArgumentParser:
    """Construct the argument parser for this entry point."""
    parser = argparse.ArgumentParser(description="Run Hirschmann health ingestion.")
    ...
    parser.add_argument("--source", default=None, help="snmp | traps (default: both).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sources = [args.source] if args.source else HEALTH_SOURCES
    status, output_dir = runtime.run(
        sources, live, load_yaml(args.sophos_config), load_yaml(args.hirschmann_config),
        mode=mode, dry_run=args.dry_run,
    )
    logger.info("Hirschmann health ingestion complete: %d events, output=%s",
